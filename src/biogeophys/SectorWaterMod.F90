...
!-----------------------------------------------------------------------
     ...
     subroutine ReadSectorWaterData (this, bounds, year, mon)
          ...
          ! This seems ineficient, I should be able to read directly into the this%input arrays without creating any new temporarry arrays like now...
          ! Also I am wondering if it wouldn't be faster to read yearly data and then access from the array the corresponing month, then reading each month separatly.
          ! Concerning the reading from surfdata, I may need to change this part.
          ! For example instead of reading from fsurdat file I can request add additionally the current year as variable, and then read from a targeted file
          ! corresponding to the data for the year X and month mon
